...
When writing or revising any prompt in this repo — a skill, an agent, this file — the reference is `.agents/writing-for-agents.md` (skill-specific mechanics in its companion file). It is what Hard Rule 8 looks like in practice: how a pointer's wording decides whether material gets reached, when to disclose behind one, and the tests that catch a rule that has quietly become a no-op.
