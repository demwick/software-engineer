...
2. **Token efficiency matters, but match the model to cognitive load — not to whether the agent writes files.** Use Haiku for the genuinely fast-turn read-only agent (`researcher`). Use Sonnet for judgment-heavy execution (`executor`) **and for `verifier`** — verification is adversarial senior review (find the failure the executor missed, classify by severity), the hardest task in the pipeline, and read-only ≠ easy. The verifier is not on the per-turn hot path: the Stop-hook check is the bash `hooks/auto-qa` loop, which runs tests directly and never spawns the agent, so the verifier agent runs only on explicit flow-driven phase verification — Sonnet there is cheap in aggregate. Use Opus for the highest-leverage planning agent (`planner`) — a flawed plan cascades into work even a strong verifier can't fully catch. Read-only agents must never get `Write` or `Edit` — use `disallowedTools` or an explicit `tools:` allowlist.
