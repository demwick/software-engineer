...
|---|---|---|---|
| `intent/<slug>.md` | `intent` skill | `spec`, the planning step | Problem, Outcome, Users, Scale, Auth, Critical NFRs, **Non-goals** (≥2), Constraints, Open questions |
| `specs/<slug>.md` | `spec` skill | the planning step, `verifier` | `## What we're building`, `## Non-goals` (≥2), `## Acceptance criteria` (≥3, testable), Edge cases, Trade-offs; `**Status:** draft → accepted`. Linted by `scripts/spec-validate.sh` |
| `plans/<id>.md` | the flows, linted then accepted with `AskUserQuestion` | `executor`, `verifier`, `verify-phase.sh` | `## Files`, `## Tasks` (≥1 `### Task`), `## Acceptance criteria` (≥2, testable), `## Risks` (`confirm: yes|no`), `## Proof`. Linted by `scripts/plan-validate.sh`. `<id>` is `phase-N` or a slug |
...
| `verification/<id>.closed.json` | `scripts/state-update.sh --close-slice` | `state-update.sh` itself, on a repeat call | `{record_version, id, slice_kind: roadmap|adhoc, from_phase, to_phase, completed, source, accepted_risk, closed_at}`. Written **before** the state update, so a second close replays the decision instead of advancing twice and an interrupt between the two is resumable |
