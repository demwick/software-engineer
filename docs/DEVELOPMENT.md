...
**Scripts** (`scripts/`): `detect-test.sh`, `detect-quality.sh`, `test-digest.sh`, `verify-phase.sh`, `write-review.sh`, `record-check.sh`, `arm-gate.sh`, `red-proof.sh`, `plan-validate.sh`, `spec-validate.sh`, `state-init.sh`, `state-update.sh`, `claude-md-init.sh`, `check-host-compat.sh`, `validate-commit-msg.sh`, `archive-state.sh`.
